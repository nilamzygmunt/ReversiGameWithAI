...
public class CornerHeuristic implements Heuristic{
    ...
    public int evaluate(GameState node, int player) {
        ...
        return 100 * (!board.isPlayer1Turn() ?  (board.getPlayer1DisksNumber() - board.getPlayer2DisksNumber())
                ...
                / (board.getPlayer1DisksNumber() + board.getPlayer2DisksNumber()) + 100 * cornerValue(board, player);

    }

    private int cornerValue(Board board, int player)
    {
        int i = 0;
        i += board.getBoard()[0][board.getWidth()-1] == player ? 1 : 0;
        i += board.getBoard()[board.getHeight()-1][0] == player ? 1 : 0;
        i += board.getBoard()[board.getHeight()-1][board.getWidth()-1] == player ? 1 : 0;
        i += board.getBoard()[0][0] == player ? 1 : 0;
